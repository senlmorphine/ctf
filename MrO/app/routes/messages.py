    ...
    if request.method == 'GET':
        ...
        if "username" in request.form:
            ...
            if message == '':
                ...
                flash('These characters are not allowed: <>[]|;', 'danger')
                return redirect(url_for('home'))
            elif "{{" in message or "}}" in message or "config" in message:
                flash('NOT AGAIN someone hacked me using the sanme characters >:(', 'danger')
                return redirect(url_for('home'))
            if msg_to == 'Send to:':
...
def ssti_index_template(msg):
    ...
    <div class="px-4 py-5 text-center">
        <h1>Send Message to Everyone!</h1>
    </div>
        ...
        <form style='margin-left: -12px; margin-right: -12px' id='form' method='POST' name="msgForm" action='messages'>    
            <blockquote>
                <!-- <div class="alert alert-danger text-wrap align-self-center alert-dismissible" id="invalidChars"
                    ...
                    These characters are not allowed:<br>
