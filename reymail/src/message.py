...
def ldap_to_reymail_id(user_cns: List[str]) -> Dict[str, IdType]:
    reymail_users = {}
    with ldap3.Connection(
        ...
        for user_cn in user_cns:
            ...
            if success:
                reymail_users[user_cn] = uuid.UUID(res[0]["attributes"]["uid"][0])
    return reymail_users
...
async def send_message(message_spec: MessageInfo, identity_provider: IdentityProviders):
    try:
        recipients = ldap_to_reymail_id(message_spec.recipients)
        reymail_recipients = list(recipients.values())
        ...
        async with redis.pipeline(transaction=True) as pipe:
            ...
            for recipient in reymail_recipients:
