...
int main(int ARGC, const char** const ARGV) {
   ...
   double forbearance = (ARGC > 1) ? atof(ARGV[1]) : 0.;

   printf("== The Umbrella Academy, Level of forbearance %.17g ==\n\n", forbearance);
   ...
   if (b > a && fabs(a) < MAXab && fabs(b) < MAXab) {
      ...
      double sgoal = -M_PI;
      if (fabs(s - sgoal) > forbearance) printf("=> NO:(, try harder...\n");
      else {
         FILE* f = fopen("./flag.txt", "r");
         char buf[256];
         fread(buf, 256, 1, f);
         fclose(f);
         if (s == sgoal) printf("=> Nice job! Here is the flag: %s\n", buf);
