...
print('Welcome to DC! Can you prove your are Flash?\n')
print(f'I\'m thinking of a number between 0 and {NIST_256_CURVE[2]}...')

while 1:
    guess_str = input("Can u guess this number (in hexadecimal):")
    try:
    ...
    if hmac.compare_digest(randval.to_bytes(256, 'big'), guess.to_bytes(256, 'big')):
        print('Excellent! it\'s correct! Maybe you are the real Flash. Here\'s your flag :.')
        with open('FLAG') as f:
            print(f.readlines()[0])
        exit(0)

    print(f'Incorrect, You\'re a fake Flash, my number was {hex(randval)}')
    print('Try again...\n')
