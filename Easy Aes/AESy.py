...
def decrypt(encoded_ct, key):
    if not len(encoded_ct) % (2*BLOCK_LEN) == 0:
        return "\nCypher: Ciphertext length is not a multiple of block length(=16)."

    if len(encoded_ct) == 32:
        return "\nCypher: You have just given me the IV. There is no encrypted message appended to it??"

    for i in encoded_ct:
        if i not in hexdigits:
            return '\nCypher: There are non-hex digits in the ciphertext.'
...
def main():
    ...
    print "\nWELCOME TO Cypher's ENCRYPTION SERVICE\n(Plaintext is hex-encoded before encryption)"
    
    while True:
        print "\n"
        print "1. Get your message encrypted."
        print "2. Leave a message for Cypher to decrypt."
